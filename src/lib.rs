...
//! Without `resample`, [`ReadConfig`] has no `sample_rate` field at all, so asking for a rate
//! nothing would resample to does not compile.
//!
//! ## Reading
//!
//! [`ReadConfig`] selects what to read. Only the selected frames are stored, though the reader
//! may decode and discard earlier packets for accurate seeking and codec warm-up.
//!
//! Positions are given as a [`Position`], in frames or in time. `start` is inclusive and `stop`
...
//! in the build can read fails with [`ReadError::UnsupportedFormat`].
//!
